...
def parse_arguments():
    ...
    parser.add_argument("--twins_config", help="Config file to use (see configs dir)", default="src/configs/pcpvt_small.yaml")
...
def get_config():
    ...
    # load yaml file
    args.twins_config = os.path.join(sys.path[0], args.twins_config)
    if args.run_modelarts:
        import moxing as mox
        with mox.file.File(args.twins_config, 'r') as f:
            yaml_txt = f.read()
    else:
        yaml_txt = open(args.twins_config).read()
    ...
    print(f"=> Reading YAML config from {args.twins_config}")
